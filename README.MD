# 🧠 BiForget: Target-Model-Guided Synthesis for LLM Unlearning
...
Unlike prior approaches that rely on external generators or fixed benchmarks, BiForget synthesizes forget data **directly guided by the target model itself**, enabling more *effective* unlearning.
...
We show that these two scenarios impose **fundamentally different data requirements**, which are often conflated in prior work.
...
BiForget integrates seed extraction, target-model-guided expansion, adversarial probing, and quality evaluation into a unified synthesis loop.
...
* Target relevance,
* Data efficiency across different unlearning algorithms.
...
In all experiments, BiForget **only replaces the forget dataset**, while keeping the original training, optimization, and evaluation protocols unchanged.
...
- **WMDP (Safety-Oriented Domain Unlearning)**  
...
If you find this repository useful, please cite our paper (to be added).
